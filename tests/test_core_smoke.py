...
try:
    ...
    def _imwrite(path, image, params=None):
        from PIL import Image
        ...
        if array.ndim == 2:
            ...
            output = Image.fromarray(array[:, :, :3].astype("uint8"))
        output.save(path)
        return True
...
if importlib.util.find_spec("yaml") is None:
    ...
    sys.modules["yaml"] = types.SimpleNamespace(safe_load=_safe_load_yaml)
...
class CoreSmokeTest(unittest.TestCase):
    def test_global_identity_status_transitions(self) -> None:
        manager = GlobalIdentityManager()

        manager.set_target(1, 42)
        ...
        manager.handle_camera_cut(2)
        self.assertEqual(manager.selected_global_vehicle_id, 1)
        self.assertEqual(manager.selected_local_track_id, -1)
        self.assertEqual(manager.status, "CameraCut")

        manager.set_target(1, -1)
        self.assertEqual(manager.status, "SearchingTarget")

        manager.reset()
        self.assertEqual(manager.selected_global_vehicle_id, -1)
        self.assertEqual(manager.selected_local_track_id, -1)
        self.assertEqual(manager.status, "Detecting")

    def test_anchor_database_matches_current_and_past_shot_features(self) -> None:
        database = HierarchicalAnchorDatabase(hysteresis_margin=0.05)
        database.add_anchor(1, 1, np.array([1.0, 0.0], dtype=np.float32), "car")

        best = database.match_candidates(
            shot_id=1,
            candidate_features=[
            ...
            target_global_id=1,
            current_local_track_id=-1,
            local_track_ids=[4, 7],
        )
        self.assertEqual(best, 7)

        fallback = database.match_candidates(
            shot_id=2,
            candidate_features=[np.array([0.99, 0.01], dtype=np.float32)],
            target_global_id=1,
            current_local_track_id=-1,
            local_track_ids=[9],
        )
        self.assertEqual(fallback, 9)

        state = database.get_state()
        self.assertEqual(state[1]["label"], "car")
        self.assertEqual(state[1]["shots"], {1: 1})
        self.assertEqual(state[1]["total_features"], 1)

    def test_anchor_database_hysteresis_keeps_current_track_when_scores_are_close(self) -> None:
        database = HierarchicalAnchorDatabase(hysteresis_margin=0.05)
        database.add_anchor(1, 1, np.array([1.0, 0.0], dtype=np.float32), "car")
    ...
    def test_control_panel_start_uses_selected_tracker(self) -> None:
        calls = []
        panel = object.__new__(ControlPanel)
        panel.source = object()
        panel.tracker = types.SimpleNamespace(get=lambda: "bytetrack")
        panel.on_start = lambda source, tracker: calls.append((source, tracker))

        ControlPanel._start(panel)

        self.assertEqual(calls, [(panel.source, "bytetrack")])

    @unittest.skipUnless(HAS_REAL_CV2, "OpenCV video I/O is not installed")
    ...
    def test_pipeline_seek_preserves_anchor_database_but_resets_runtime_target(self) -> None:
        class FakeSource:
        ...
        worker.anchor_db.add_anchor(1, 1, np.array([1.0, 0.0], dtype=np.float32), "car")
        worker.identity_manager.set_target(1, 7)

        worker._seek_source(42)

        self.assertEqual(source.seek_calls, [42])
        self.assertEqual(detector.reset_count, 1)
        self.assertEqual(worker.anchor_db.get_state()[1]["total_features"], 1)
        self.assertEqual(worker.identity_manager.selected_global_vehicle_id, -1)
        self.assertEqual(worker.identity_manager.selected_local_track_id, -1)
    ...
    def test_prepare_reid_dataset_from_yolo_writes_class_identity_crops(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            Image.new("RGB", (120, 100), (255, 255, 255)).save(image_path)
    ...
    def test_build_vehicle_identity_candidates_writes_track_crops(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for index in range(2):
                Image.new("RGB", (100, 80), (255, 255, 255)).save(source_dir / f"frame_{index:03d}.jpg")
    ...
    def test_fastreid_onnx_wrapper_scales_and_normalizes_embeddings(self) -> None:
        ...
        from scripts import export_fastreid_reid_onnx
